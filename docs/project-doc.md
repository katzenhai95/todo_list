...
| F-009 | 屏幕边缘自动隐藏 | 将窗口拖至屏幕左/右边缘附近（15px 内）→ 停止拖动后窗口自动吸附紧贴边缘 → 鼠标离开窗口后，窗口自动滑出屏幕（留 3px 把手）。仅左右边缘生效，上下边缘不触发隐藏。 | 拖窗口靠近屏幕左边缘（15px 内）→ 停止拖动 → 窗口吸附到边缘（wx=0）→ 鼠标移开 → 0.4 秒后窗口滑出 → 仅剩 3px 边缘可见 |
| F-010 | 边缘弹出恢复 | 当窗口隐藏于左/右边缘时，鼠标移到对应边缘的把手位置 → 窗口自动滑回原位 | 窗口隐藏后 → 鼠标移向屏幕左/右边缘把手 → 窗口滑出完整显示 |
...
| F-013 | 置顶切换 | 窗口工具栏提供"置顶"按钮，点击后窗口始终在最前（不被其他窗口遮挡）；再次点击取消置顶，恢复普通窗口层级。默认启动时为置顶状态。 | 打开其他窗口覆盖待办清单 → 点击"置顶"按钮 → 窗口浮到最前 → 再次点击 → 窗口可被覆盖 |
...
| 层次 | 位置 | 实现要点 |
|------|------|----------|
| 状态标志 | `app.py` — `TodoApp._topmost` | 布尔标志，`True` 表示窗口置顶。初始化时设为 `True`（默认置顶）。 |
| 系统调用 | `app.py` — `_toggle_topmost()` | 翻转 `_topmost` → 调用 `self.attributes("-topmost", ...)` 通知窗口管理器。 |
| 按钮 UI | `app.py` — `_topmost_btn` | 工具栏 CTkButton，文字动态切换：置顶态显示"取消置顶"（指示可执行的操作），非置顶态显示"置顶"。 |

---

## 四、测试用例表

测试用例按功能点 (F-001 ~ F-013) 组织，分为**自动化测试**（`pytest` 一键运行，结果自动判定）和**人工验证**（需用户交互观察，用例中注明判定标准）。
...
| TC-F009-M | F-009 屏幕边缘自动隐藏 | 拖动吸附 + 左/右边缘隐藏 | 1. 拖动窗口靠近屏幕左边缘（15px 内）2. 停止拖动 3. 鼠标移出窗口 4. 等待 0.4 秒 | 用户自行检查：(a) 停止拖动后窗口自动吸附紧贴左边缘；(b) 鼠标移开后窗口向左侧滑出，仅留约 3px 把手。右边缘行为一致。上下边缘不触发隐藏。 |
| TC-F010-M | F-010 边缘弹出恢复 | 隐藏后鼠标触发弹出 | 1. 确认窗口已隐藏至左边缘 2. 将鼠标移向屏幕左边缘的 3px 把手区域 | 用户自行检查窗口是否向右滑出并完整显示。右边缘恢复行为一致。 |
...
| TC-F013-M | F-013 置顶切换 | 置顶按钮切换窗口层级 | 1. 打开另一个窗口（如文件管理器），将其覆盖在待办清单窗口上 2. 点击待办清单工具栏的"置顶"按钮 3. 观察窗口是否浮到最前 4. 再次点击"置顶"按钮 5. 用其他窗口覆盖待办清单 | 用户自行检查：(a) 点击置顶后窗口浮到所有窗口上方；(b) 再次点击后窗口可被其他窗口覆盖；(c) 按钮文字在"置顶"和"取消置顶"之间切换。 |
| TC-F022-M | 边界测试 | 窗口在非边缘位置不触发隐藏 | 1. 将窗口放在屏幕中央 2. 鼠标离开窗口 | 用户自行检查窗口不执行隐藏动画。 |

---

*文档版本: 1.0.0 — 最后更新: 2026-05-24*
